  ...
  return new Promise((resolve, reject) =>
    requestPhaseList.run(ctx, (requestPhasesError) => {
      ...
      if (requestPhasesError) {
        // check if we got a response from caching plugin
        if (!_.isError(requestPhasesError)) {
          debug('requestPhaseLists aborted with a response: %o and body: %o', requestPhasesError.response, requestPhasesError.body); // eslint-disable-line max-len

          // resolve with a readable stream instance in its basic form,
          // which indicates that the http request has not been made
          // since otherwise Promise resolves to a duplex stream
          resolve(new Readable());
          callback(null, requestPhasesError.response, requestPhasesError.body);
          return;
        }
