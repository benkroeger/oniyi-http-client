...
const makeRequest = (
  instance,
  { request: requestPhaseList, response: responsePhaseList },
  ...args
) => {
  // augment requestParams to properly handle absolute uris
  // merge params with defaults
  const requestOptions = parseUri(
    _.defaultsDeep(
  ...
  return new Promise((resolve, reject) =>
    requestPhaseList.run(ctx, (requestPhasesError) => {
      ...
      _.set(
        ctx,
        'options.callback',
        (responseError, response, responseBody) => {
          /**
          ...
          responsePhaseList.run(responseCtx, (responsePhasesError) => {
            ...
            callback(
              responseCtx.responseError,
              responseCtx.response,
              responseCtx.responseBody,
            );
          });
        },
      );
