...
const { removePhasesByOptions } = require('./phase-list');
...
const makeRequest = (instance, phaseLists, ...args) => {
  ...
  const { request: requestPhaseList, response: responsePhaseList } = removePhasesByOptions(phaseLists, requestOptions);
