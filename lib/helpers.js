...
function applyPlugins(plugins, params, callback) {
  params.pluginData = params.pluginData || {};
  async.reduce(plugins, params, function(currentParams, plugin, cb) {
    ...
    params.pluginData[plugin.name] = params.pluginData[plugin.name] || {};
    ...
    return cb(new Error(util.format('unkonwn signature for "onRequest" in plugin "%s"', plugin.name)));
  }, function(err, params) {
    if (err === 'abortRequest') {
      return callback(null, _.omit(params, ['response', 'raw', 'parsed', 'expireAt']), params.response, params.raw);
    }
    callback(err, params);
  });
}
...
function makeRequestCallback(plugins, pluginData, originalCallback) {
  ...
  function next() {
    ...
    args.unshift(next);
    // @TODO: does it set the "this" scope correctly?
