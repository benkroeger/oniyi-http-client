...
import { initPhaseList } from '../phase-list';
...
test('throws when plugin registers handler for non-existing phase name', (t) => {
  ...
  t.is(error.message, `can not mount handler from plugin ${plugin.name}. request phase ${hook1.phaseName} is unknown`);
});
