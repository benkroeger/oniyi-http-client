...
test('mountPlugin is a function', t => {
  t.true(_.isFunction(mountPlugin));
});
...
test('throws when plugin is not a plain object', t => {
  ...
  const error = t.throws(
    () => mountPlugin(httpClient, phaseLists, false),
    TypeError,
  );
...
test('throws when plugin.name is not a string', t => {
  ...
  const error = t.throws(
    () => mountPlugin(httpClient, phaseLists, { name: false }),
    TypeError,
  );
...
test.cb('registered handlers are executed in correct order', t => {
  ...
  mountPlugin(httpClient, phaseLists, {
    name: 'plugin1',
    onRequest: [hook1, hook2, hook3, hook4],
  });
  ...
  mountPlugin(httpClient, phaseLists, {
    name: 'plugin1',
    onRequest: [hook5, hook6, hook7, hook8],
  });
