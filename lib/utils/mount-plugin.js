...
module.exports = (httpClient, phaseLists, plugin, options = {}) => {
  ...
  [
    ...
    hooksArray.forEach(({ phaseName, handler }) => {
      const [, mainPhaseName = phaseName, subphase = 'use'] =
        phaseName.match(phaseNameRegex) || [];
      const mappedPhaseName = _.get(
        options,
        `${listName}PhaseMap.${mainPhaseName}`,
        mainPhaseName,
      );
      ...
      if (!phase) {
        throw new Error(
          `can not mount handler from plugin ${plugin.name}. ${listName} phase ${phaseName} is unknown`,
        );
      }

      phase[subphase]((ctx, next) => {
        debug(
          'executing %s handler for plugin %s in phase %s',
          listName,
          plugin.name,
          phaseName,
        );
        handler(ctx, (...args) => {
          debug(
            'finished %s handler execution for plugin %s in phase %s',
            listName,
            plugin.name,
            phaseName,
          );
      ...
      debug(
        'registered %s handler for plugin %s in phase %s',
        listName,
        plugin.name,
        phaseName,
      );
