...
import { initPhaseList } from '../lib/phase-list';
...
test('removes the phase from a cloned phaseList', async (t) => {
  const { httpClient, phaseLists } = t.context;
  const { request: { _phases: phases } } = phaseLists;

  const spy = sinon.spy(phaseLists.request, 'remove');
  ...
  // we have only removed a phase from a cloned version,
  // original phaseList should stay unchanged
  t.is(phases.length, 2);
  t.true(spy.calledOnce);
});
