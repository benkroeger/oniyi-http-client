...
import mountPlugin from '../lib/helpers/mount-plugin';
...
test.cb('responds with cached data if available', (t) => {
  const { httpClient, phaseLists } = t.context;
  ...
  const requestSpy = sinon.spy(phaseLists.request, 'run');
  const responseSpy = sinon.spy(phaseLists.response, 'run');

  mountPlugin(httpClient, phaseLists, {
    name: 'mock-cache-plugin',
    onRequest: [
      {
        phaseName: 'initial',
        handler: (ctx, next) => next(pluginError),
  ...
  const options = {
    ...
    callback: (err, response, body) => {
      ...
      t.is(requestSpy.callCount, 1);
      t.is(responseSpy.callCount, 0);
      t.end();
    },
  };
  ...
  makeRequest(httpClient, phaseLists, options)
    .then((readStream) => {
      t.true(readStream instanceof Readable);
    });
});
