...
test('makeRequest is a function', t => {
  t.true(_.isFunction(makeRequest));
});
...
test('rejects returned Promise with error from request phase list', async t => {
  ...
  const thrown = await t.throwsAsync(() => makeRequest(httpClient, phaseLists));
...
test.cb(
  'invokes options.callback results from response phase list context',
  t => {
    const { httpClient, phaseLists } = t.context;
